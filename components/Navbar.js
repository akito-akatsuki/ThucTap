    ...
    <>
      ...
      {isAnimating && (
        <>
          {/* Backdrop */}
          <div
            className={`fixed inset-0 z-[998] bg-black/40 transition-opacity duration-200 ease-out ${showLoginForm ? "opacity-100" : "opacity-0 pointer-events-none"}`}
          ...
          >
            <div
              ...
              className={`relative w-full max-w-xl md:w-[420px] bg-white/95 dark:bg-slate-900/95 rounded-3xl shadow-2xl border border-white/50 dark:border-slate-700 p-8 transition-all duration-200 ease-out transform origin-top-right ${showLoginForm ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-95 translate-y-4"} max-h-[calc(100vh-4rem)] overflow-y-auto`}
            >
